...
# (Asegúrate de colocar tu clave key anon o service_role pública aquí)
...
async def agregar_producto(
    ...
    try:
        # 1. Subir la imagen directamente al Bucket "productos" de Supabase Storage
        ...
        upload_response = requests.post(storage_url, headers=storage_headers, data=file_bytes)
        
        # Obtener la URL pública permanente de la imagen en Supabase
        imagen_url = f"{SUPABASE_URL}/storage/v1/object/public/productos/{file_name}"
        
        # 2. Guardar los datos del producto y la URL de la imagen en la base de datos
        payload = {
            "nombre": nombre,
            "precio": float(precio),
            "categoria": categoria,
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.environ.get("PORT", 8000)))import os
from fastapi import FastAPI, Request, Form, UploadFile, File, WebSocket, WebSocketDisconnect
from fastapi.responses import HTMLResponse, RedirectResponse
from fastapi.staticfiles import StaticFiles
from fastapi.templating import Jinja2Templates
